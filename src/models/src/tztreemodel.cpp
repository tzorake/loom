...
void TzTreeModel::connectNode(const TzTreeNodePtr& node)
{
    ...
    conn.onChildAdded = TzScopedEventListener(
        node->events().on("childAdded", [this, nodeWeak = std::weak_ptr(node)]
            (TzTreeNodePtr child, int index) {
    ...
    conn.onChildRemoved = TzScopedEventListener(
        node->events().on("childAboutToBeRemoved", [this, nodeWeak = std::weak_ptr(node)]
            (TzTreeNodePtr child, int index) {
            auto node = nodeWeak.lock();
            if (!node) return;
    ...
    conn.onNameChanged = TzScopedEventListener(
        node->events().on("nameChanged", [this, nodeWeak = std::weak_ptr(node)]
            (const std::string&) {
            auto node = nodeWeak.lock();
            if (!node) return;
