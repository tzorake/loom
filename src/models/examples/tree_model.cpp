...
static void printModel(const TzTreeModel& model,
                       const TzModelIndex& parent = TzModelIndex(),
                       int depth = 0)
{
...
int main()
{
    ...
    auto onInserted = model.events().on("rowsInserted",
        [&](const TzModelIndex& parent, int first, int last) {
            auto parentName = parent.isValid()
                ? std::any_cast<std::string>(model.data(parent))
                : std::string("<root>");
            std::println("  [+] rows {}..{} inserted under \"{}\"",
                         first, last, parentName);
        });
    ...
    auto onRemoved = model.events().on("rowsRemoved",
        [&](const TzModelIndex& parent, int first, int last) {
            auto parentName = parent.isValid()
                ? std::any_cast<std::string>(model.data(parent))
                : std::string("<root>");
            std::println("  [-] rows {}..{} removed from \"{}\"",
                         first, last, parentName);
        });

    auto onChanged = model.events().on("dataChanged",
        [&](const TzModelIndex& topLeft, const TzModelIndex& /*bottomRight*/,
            const std::vector<int>& /*roles*/) {
            auto newName = std::any_cast<std::string>(model.data(topLeft));
            std::println("  [~] row {} renamed to \"{}\"",
                         topLeft.row(), newName);
        });
