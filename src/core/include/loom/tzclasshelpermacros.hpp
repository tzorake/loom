...
template <typename T> inline T *tzGetPtrHelper(T *ptr) noexcept { return ptr; }
template <typename Ptr> inline auto tzGetPtrHelper(Ptr &ptr) noexcept -> decltype(ptr.get())
{ static_assert(noexcept(ptr.get()), "Smart d pointers for TZ_DECLARE_PRIVATE must have noexcept get()"); return ptr.get(); }
