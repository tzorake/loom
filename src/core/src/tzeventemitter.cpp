...
TzEventListener::TzEventListener(std::weak_ptr<TzEventEmitterPrivate> impl, std::string event,
                                 std::uint64_t id)
...
std::pair<std::uint64_t, std::string> TzEventEmitterPrivate::addListener(
    const std::string &eventName, ListenerCallback callback)
{
...
std::pair<std::uint64_t, std::string> TzEventEmitterPrivate::addOnceListener(
    const std::string &eventName, ListenerCallback callback)
{
    std::uint64_t id = nextId++;
    auto onceFn = [weakImpl = weak_from_this(), eventName, id,
                   callback = std::move(callback)](const std::vector<std::any> &args) {
        callback(args);
...
void TzEventEmitterPrivate::checkLimit(const std::string &event, std::size_t count)
{
    if (count > maxListeners) {
        std::cerr << std::format(
            ...
            "{} listeners on event '{}'. Use setMaxListeners() to increase limit.",
            count, event)
                  << std::endl;
    }
}
