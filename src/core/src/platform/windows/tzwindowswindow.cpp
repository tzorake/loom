...
{
    KeyModifiers mods;
    if (GetKeyState(VK_SHIFT)   & 0x8000) mods |= KeyModifier::Shift;
    if (GetKeyState(VK_CONTROL) & 0x8000) mods |= KeyModifier::Ctrl;
    if (GetKeyState(VK_MENU)    & 0x8000) mods |= KeyModifier::Alt;
    return mods;
}
...
{
    KeyModifiers mods;
    if (wParam & MK_SHIFT)   mods |= KeyModifier::Shift;
    if (wParam & MK_CONTROL) mods |= KeyModifier::Ctrl;
    return mods;
}
...
void TzWindowsWindowPrivate::onKey(WPARAM vk, bool pressed, LPARAM lParam)
{
    ...
    TzCoreApplication::postEvent(owner,
        new TzKeyEvent(
            pressed ? TzEvent::KeyPress : TzEvent::KeyRelease,
...
void TzWindowsWindowPrivate::onMouseButton(UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg) {
        case WM_LBUTTONDOWN: button = MouseButton::Left;   type = TzEvent::MouseButtonPress;   break;
        case WM_LBUTTONUP:   button = MouseButton::Left;   type = TzEvent::MouseButtonRelease; break;
        case WM_RBUTTONDOWN: button = MouseButton::Right;  type = TzEvent::MouseButtonPress;   break;
        case WM_RBUTTONUP:   button = MouseButton::Right;  type = TzEvent::MouseButtonRelease; break;
        case WM_MBUTTONDOWN: button = MouseButton::Middle; type = TzEvent::MouseButtonPress;   break;
        case WM_MBUTTONUP:   button = MouseButton::Middle; type = TzEvent::MouseButtonRelease; break;
        default: break;
    }
...
void TzWindowsWindowPrivate::onMouseMove(LPARAM lParam, WPARAM wParam)
{
    ...
    double y = static_cast<double>(GET_Y_LPARAM(lParam));
    TzCoreApplication::postEvent(owner,
        new TzMouseEvent(TzEvent::MouseMove, MouseButton::None, x, y,
